...
function EnvVarRow({
  ...
  const safeInfoUrl = sanitizeExternalUrl(info.url);
    ...
    return (
      <div className="flex items-center justify-between gap-3 py-1.5 opacity-50 hover:opacity-100 transition-opacity">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {safeInfoUrl && (
            <a href={safeInfoUrl} target="_blank" rel="noreferrer"
...
export default function EnvPage() {
  ...
  const loadEnvVars = () => {
    ...
    api
      .getEnvVars()
      .then(setVars)
      .catch(() => setLoadError("Failed to load environment variable metadata."));
  };
    ...
    if (loadError) {
      return (
        <div className="mx-auto max-w-2xl border border-destructive/30 bg-destructive/[0.06] p-6">
          ...
          <Button className="mt-4" variant="outline" onClick={loadEnvVars}>Retry</Button>
        </div>
      );
    }

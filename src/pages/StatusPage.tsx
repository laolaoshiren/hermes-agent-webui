...
export default function StatusPage() {
  ...
  useEffect(() => {
    const load = () => {
      setLoadError(null);
      api.getStatus().then(setStatus).catch(() => setLoadError("Failed to load Hermes status."));
      api.getSessions().then(setSessions).catch(() => setLoadError((prev) => prev ?? "Failed to load session summary."));
    };
    load();
    ...
    if (loadError) {
      return (
        <div className="mx-auto max-w-2xl border border-destructive/30 bg-destructive/[0.06] p-6">
          ...
          <div className="mt-2 text-xs text-destructive/80">Check whether the Hermes backend is reachable, then refresh this page.</div>
        </div>
      );
    }
